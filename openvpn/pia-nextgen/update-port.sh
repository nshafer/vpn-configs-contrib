...
setup_tls() {
  ...
  if [[ -z "$pf_cn" ]]; then
    fatal_error "PIA: $CONFIG has no '; pia_cn' line, so the port forwarding API certificate cannot be verified.
Configs generated by this provider always carry one. If you supplied your own
config, regenerate it or set PIA_PF_INSECURE=true to connect without verifying."
  fi
  if [[ ! -f "$cacert" ]]; then
    fatal_error "PIA: $cacert is missing, so the port forwarding API certificate cannot be verified.
Set PIA_PF_INSECURE=true to connect without verifying."
  fi
...
find_api_host() {
  ...
  fatal_error "PIA: could not reach a port forwarding API from gateway $pf_gateway.
The tunnel is up, but region '$pf_region' is not serving port forwards. Check
that it still has port_forward=true in the server list at
https://serverlist.piaservers.net/vpninfo/servers/v6 -- and note that some
regions advertise port forwarding but do not actually serve it, so it is worth
trying another region too."
}
...
get_token() {
  ...
  if [[ -z "$pf_token" ]]; then
    fatal_error "PIA: could not get an account token, check the credentials in $OPENVPN_CREDENTIALS.
Response: $response"
  fi
...
request_signature() {
  ...
  if [[ "$(jq -r '.status // empty' <<< "$response")" != "OK" ]]; then
    fatal_error "PIA: getSignature failed at ${pf_api_ip}:${PF_API_PORT} for region '$pf_region'.
Some regions advertise port forwarding but do not actually serve it, so it is
worth trying another region.
Response: ${response:-<empty>}"
  ...
  if [[ -z "$pf_payload" || -z "$pf_signature" ]]; then
    fatal_error "PIA: getSignature returned no payload or signature.
Response: $response"
  fi
  ...
  if [[ -z "$pf_port" || -z "$expires_raw" ]]; then
    fatal_error "PIA: could not read a port and expiry out of the signed payload.
Payload: ${decoded:-<undecodable>}"
  fi
...
bind_port() {
  ...
  if [[ "$(jq -r '.status // empty' <<< "$response")" != "OK" ]]; then
    log "PIA: bindPort failed at ${pf_api_ip}:${PF_API_PORT}. Response: ${response:-<empty>}"
    return 1
  fi
...
main() {
  ...
  while true; do
    ...
    if ! bind_port; then
      fatal_error "PIA: lost the port forward for $pf_port on ${pf_api_ip}:${PF_API_PORT}.
The reservation is gone, so the container is stopping rather than leaving
